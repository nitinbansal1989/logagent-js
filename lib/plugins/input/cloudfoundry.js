...
class InputCloudFoundry {
    constructor(config, eventEmitter) {
        ...
        logger_js_1.default.log('value ' + this.config.cfhttp);
    ...
    getHttpServer(aport, handler) {
        ...
        try {
            ...
            logger_js_1.default.log('Logagent listening (http): ' + _port + ', process id: ' + process.pid);
            return server;
        }
        catch (err) {
        ...
        try {
            ...
            if (self.config && self.config.blacklist && self.config.blacklist[token]) {
                if (self.config.debug) {
                    logger_js_1.default.log('blacklisted request for' + token);
                }
                res.statusCode = 404;
                return res.end();
            }
            ...
            logger_js_1.default.error('Error in Cloud Foundry (http): ' + err);
        }
    }
    startCloudfoundryServer(id) {
        logger_js_1.default.log('start cloudfoundry worker: ' + id + ', pid:' + process.pid);
        ...
        var terminate = function (reason) {
            return function () {
                ...
                exitInProgress = true;
                logger_js_1.default.log('stop Cloud Foundry worker: ' + id + ', pid:' + process.pid + ', terminate reason: ' + reason + ' memory rss: ' + (process.memoryUsage().rss / (1024 * 1024)).toFixed(2) + ' MB');
...
exports.default = InputCloudFoundry;
