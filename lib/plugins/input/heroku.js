...
class InputHeroku {
    ...
    herokuHandler(req, res) {
        try {
            ...
            logger_js_1.default.error('Error in Heroku (http): ' + err);
        }
    }
    startHerokuServer(id) {
        logger_js_1.default.log('start heroku worker: ' + id + ', pid:' + process.pid);
        ...
        var terminate = function (reason) {
            return function () {
                ...
                exitInProgress = true;
                logger_js_1.default.log('stop heroku worker: ' + id + ', pid:' + process.pid + ', terminate reason: ' + reason + ' memory rss: ' + (process.memoryUsage().rss / (1024 * 1024)).toFixed(2) + ' MB');
